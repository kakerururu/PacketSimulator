...
def main():
    ...
    detection_records = read_detector_logs()
    ...
    # 3. グループ化されたレコードをCSV出力
    print("\n[Phase 3] グループ化されたレコードをCSV出力中...")
    export_result = export_grouped_records(grouped_records)
    print(f"✓ 出力完了: {export_result['num_payloads']} ファイル")
    print(f"  出力先: src2_result/grouped_records/")
    ...
    estimation_result = export_clustering_results(
        ...
        output_dir="src2_result/clustering_results/final"
    )
    print(f"✓ 出力完了: {estimation_result['num_payloads']} ファイル")
    print(f"  出力先: src2_result/clustering_results/final/")
    ...
    write_estimated_trajectories(
        estimated_trajectories, estimation_method="trajectory_estimation"
    )
    print("✓ 推定結果JSON出力完了: src2_result/estimated/trajectories.json")
    ...
    if estimation_result['total_unjudged'] > 0:
        ...
        print("   → 詳細は src2_result/clustering_results/final/ の Is_Judged=False を確認")


if __name__ == "__main__":
    # python -m src2.estimator.main
    main()
